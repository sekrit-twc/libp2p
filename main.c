...
static void test_api()
{
	...
	for (int i = 0; i < p2p_packing_max; ++i) {
...
static void test_rgb24_be()
{
	...
	p2p_select_pack_func(p2p_rgb24_be)(src_p, dst, 0, 1);
	printf("packed: %x %x %x\n", dst[0], dst[1], dst[2]);
...
static void test_rgb24_le()
{
	...
	p2p_select_pack_func(p2p_rgb24_le)(src_p, dst, 0, 1);
	printf("packed: %x %x %x\n", dst[0], dst[1], dst[2]);
...
static void test_y410()
{
	...
	p2p_select_pack_func(p2p_y410)(src_p, dst, 0, 1);
...
static void test_uyvy()
{
	...
	p2p_select_pack_func(p2p_uyvy)(src_p, dst, 0, 2);
